...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(({ tooltip, unstyled, ...props }, ref) => {
  const element = <button {...props} className={cx(props.className, !unstyled && buttonClassName)} ref={ref} />;
  return tooltip ?
      (
...
export const LinkButton = forwardRef<HTMLAnchorElement, LinkButtonProps>(
  ({ tooltip, unstyled, href, ...props }, ref) => {
    if (typeof href === "string" && /^(?:https?:)?\/\/|^#|\.[\da-z]+$/i.test(href)) {
      const element = (
        <a {...props} className={cx(props.className, !unstyled && buttonClassName)} href={href} ref={ref} />
      );
      return tooltip ?
          (
            <Tippy animation="shift-away" content={props["aria-label"]} {...(typeof tooltip === "boolean" ? {} : tooltip)}>
      ...
      const element = (
        <Link {...props} className={cx(props.className, !unstyled && buttonClassName)} href={href} ref={ref} />
      );
      return tooltip ?
          (
            <Tippy animation="shift-away" content={props["aria-label"]} {...(typeof tooltip === "boolean" ? {} : tooltip)}>
