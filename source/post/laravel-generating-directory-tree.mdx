...
![](https://blog.ixk.me/wp-content/uploads/2019/03/image.png)
...
\[prism lang="php"\]namespace App\\Http\\Models;
...
\[prism lang="php"\]namespace App\\Http\\Controllers;
...
\*/
public $out;
    public function get\_folders($dir)
{
global $out;
        $folder = new FolderModel();
        $arr = $folder->get\_folders($dir);
$this->print\_folders($arr, "");
return $out;
    }
    /\*\*
     \* 将文件夹树生成html
     ...
     \*/
    public function print\_folders($arr)
{
global $out;
        if($arr !== \[\]) {
...
if(strrpos($file, '/') !== false) {
                        $file\_name = substr($file, strrpos($file, '/') + 1);
                    } else {
                        $file\_name = $file;
                    }
                    if(preg\_match("/(.txt|.md)/i", $file\_name)) {
                        $file = str\_replace('uid\_'.Auth::id().'/', '', $file);
                        $li\_id = str\_replace('.', '', $file);
                        $li\_id = str\_replace('/', '', $li\_id);
                        $out .= '<li dir="'.$file.'" id="'.$li\_id.'" class="dc"><a href="#"><i class="icon icon-file-text"></i>'.$file_name.'</a></li>';
}
} else {
$dir = str\_replace('uid\_'.Auth::id().'/', '', $dir);
                    $dir\_old = $dir;
                    if(strrpos($dir, '/') !== false) {
$dir = substr($dir, strrpos($dir, '/') + 1);
                    }
                    $out .= '<li dir="'.$dir_old.'" class="dp"><a href="#"><i class="icon icon-folder-close"></i>'.$dir.'</a><ul>';
                    $out .= $this->print\_folders($file);
$out .= '</ul>';
}
}
}
